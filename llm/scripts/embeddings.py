...
if __name__ == "__main__":
    print("Arguments received:", sys.argv)  # This helps to debug the actual input received
    if len(sys.argv) > 2:
        quotedDirectory = sys.argv[1]
        config_path = sys.argv[2].strip('"')  # Correctly reference config path as the second argument
        config = read_config(config_path)
    else:
        print("Not enough arguments provided.")

    
    chunk_size = config.get('chunk_length', 4000) # Should be between 1 and n_ctx, consider the amount of RAM of your Apple Silicon Chip.
    chunk_overlap = config.get('chunk_overlap', 2000) # Should be between 1 and n_ctx, consider the amount of RAM of your Apple Silicon Chip.
